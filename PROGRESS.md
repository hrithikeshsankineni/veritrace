...
- [~] **1.2 Parsers.** IN PROGRESS (claude-code) `ingest/parsers.py` for PDF, Markdown, plain text → normalized text + metadata. *DoD:* each format parses to text with metadata in a test.
